...
<head>
    ...
    <title>@yield('title', 'HoaxChecker Indonesia')</title>
    ...
    <nav class="nav-floating fixed top-0 left-0 right-0 z-50" :class="scrolled && 'scrolled'"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:flex items-center gap-8">
                    ...
                    <a href="{{ route('hoax.check') }}" class="nav-link {{ request()->routeIs('hoax.*') ? 'nav-link-active' : '' }}">Cek Hoaks</a>
                    <a href="{{ route('education.index') }}" class="nav-link {{ request()->routeIs('education.*') ? 'nav-link-active' : '' }}">Edukasi</a>
                    @auth
                        <a href="{{ route('history.index') }}" class="nav-link {{ request()->routeIs('history.*') ? 'nav-link-active' : '' }}">Riwayat</a>
                        <a href="{{ route('report.create') }}" class="nav-link" style="color: #EF4444;">⚑ Lapor</a>
                    @endauth
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form action="{{ route('news.index') }}" method="GET" class="hidden lg:flex">
                        <div class="relative">
                    ...
                    @auth
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open"
                                ...
                                <span class="hidden sm:block text-sm font-medium text-ink-700 dark:text-ink-300">{{ auth()->user()->name }}</span>
                            ...
                            <div x-show="open" @click.outside="open = false" x-transition
                                ...
                                <div class="px-4 py-2.5 border-b" style="border-color: rgba(0,0,0,0.06);">
                                    <p class="text-xs font-semibold text-ink-800 dark:text-ink-200">{{ auth()->user()->name }}</p>
                                    <p class="text-xs text-ink-400">{{ auth()->user()->email }}</p>
                                </div>
                                <a href="{{ route('dashboard') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg>
                                    Dashboard
                                </a>
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    Profil Saya
                                </a>
                                @if(auth()->user()->role === 'admin')
                                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm transition-colors" style="color: var(--gold);">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-semibold text-ink-700 dark:text-ink-300 hover:text-ink-900 dark:hover:text-ink-50 transition-colors">Masuk</a>
                        <a href="{{ route('register') }}" class="btn-primary text-sm px-5 py-2">Daftar Gratis</a>
                    @endauth
        ...
        <div x-show="mobileOpen" x-transition class="md:hidden border-t dark:border-ink-800" style="border-color: rgba(0,0,0,0.06);">
            <div class="max-w-7xl mx-auto px-4 py-4 space-y-1">
                ...
                <a href="{{ route('news.index') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">📰 Berita</a>
                <a href="{{ route('hoax.check') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">🔍 Cek Hoaks</a>
                <a href="{{ route('education.index') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">🎓 Edukasi</a>
                @auth
                    <a href="{{ route('history.index') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-ink-700 dark:text-ink-300 hover:bg-black/04 dark:hover:bg-white/06 transition-colors">📜 Riwayat</a>
                    <a href="{{ route('report.create') }}" class="block px-3 py-2.5 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors">⚑ Laporkan Berita</a>
                ...
                @else
                    <div class="flex gap-2 pt-2">
                        <a href="{{ route('login') }}" class="flex-1 text-center py-2.5 text-sm font-semibold border border-ink-200 dark:border-ink-700 rounded-lg text-ink-700 dark:text-ink-300 transition-colors">Masuk</a>
                        <a href="{{ route('register') }}" class="flex-1 text-center py-2.5 text-sm font-bold rounded-lg text-ink-900" style="background: linear-gradient(135deg, #D4AF37, #F0D060);">Daftar</a>
                    </div>
                @endauth
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        @foreach([['Cek Hoaks', 'hoax.check'], ['Berita Terkini', 'news.index'], ['Edukasi Digital', 'education.index'], ['Lapor Berita', 'report.create']] as [$label, $route])
                        <li><a href="{{ route($route) }}" class="text-sm text-ink-500 dark:text-ink-400 hover:text-ink-900 dark:hover:text-ink-100 transition-colors">{{ $label }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        @foreach([['Tentang Kami', 'about'], ['Hubungi Kami', 'contact']] as [$label, $route])
                        <li><a href="{{ route($route) }}" class="text-sm text-ink-500 dark:text-ink-400 hover:text-ink-900 dark:hover:text-ink-100 transition-colors">{{ $label }}</a></li>
                        @endforeach
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        @foreach(['Kebijakan Privasi', 'Syarat & Ketentuan', 'Cookie'] as $item)
                        <li><a href="#" class="text-sm text-ink-500 dark:text-ink-400 hover:text-ink-900 dark:hover:text-ink-100 transition-colors">{{ $item }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <div class="border-t border-ink-100 dark:border-ink-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-sm text-ink-400">
                    &copy; {{ date('Y') }} <span class="font-semibold text-ink-700 dark:text-ink-300">HoaxChecker Indonesia</span>. Semua hak cipta dilindungi.
                </p>
