@extends('layouts.app')
@section('title', 'Dashboard Saya — HoaxChecker')
@section('content')
...
<div class="relative rounded-2xl overflow-hidden p-8 md:p-10 mb-8" style="background: linear-gradient(135deg, #0D0D0D 0%, #1A1A1A 100%);">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative">
        ...
        <a href="{{ route('hoax.check') }}" class="btn-primary flex-shrink-0">
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="card p-5 group">
        <div class="flex items-center justify-between mb-4">
            <p class="text-xs font-semibold text-ink-400 uppercase tracking-wider">{{ $stat['label'] }}</p>
        ...
        <div class="stat-number {{ isset($stat['is_text']) ? 'text-xl' : 'text-3xl' }} mb-1" style="color: {{ $stat['color'] }};">
            {{ $stat['value'] }}
        </div>
        <p class="text-xs text-ink-400">{{ $stat['sub'] }}</p>
    </div>
    @endforeach
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card p-6">
            <div class="flex items-center justify-between mb-5">
                ...
                @if(auth()->user()->newsHistories()->count() > 0)
                    <a href="{{ route('history.index') }}" class="text-xs font-semibold transition-colors" style="color: var(--gold);">
                        Lihat semua →
                    </a>
                @endif
            </div>

            @php $recentChecks = auth()->user()->newsHistories()->latest()->take(5)->get(); @endphp
            ...
            <div class="divide-y" style="--tw-divide-opacity: 1; border-color: rgba(0,0,0,0.04);">
                ...
                <div class="py-3.5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 -mx-2 px-2 rounded-lg hover:bg-black/02 dark:hover:bg-white/03 transition-colors duration-200">
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-ink-800 dark:text-ink-100 truncate text-sm">
                            {{ Str::limit($check->title, 55) }}
                        </p>
                        <p class="text-xs text-ink-400 mt-0.5">{{ $check->created_at->diffForHumans() }}</p>
                    </div>
                    <div class="flex items-center gap-3 flex-shrink-0">
                        <span class="badge badge-{{ $check->status == 'hoax' ? 'hoax' : ($check->status == 'valid' ? 'valid' : 'verify') }}">
                            {{ $check->status == 'hoax' ? '⚠ Hoaks' : ($check->status == 'valid' ? '✓ Valid' : '? Perlu Verifikasi') }}
                        </span>
                        <a href="{{ route('hoax.result', $check->id) }}"
                ...
                <div class="py-10 text-center">
                    ...
                    <a href="{{ route('hoax.check') }}" class="text-sm font-semibold transition-colors" style="color: var(--gold);">Mulai sekarang →</a>
                </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="space-y-6">
        ...
        <div class="card p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3 p-3 rounded-xl transition-all duration-200 {{ $badge['unlocked'] ? '' : 'opacity-40' }}"
                     style="{{ $badge['unlocked'] ? 'background: ' . $badge['color'] . '10; border: 1px solid ' . $badge['color'] . '25;' : 'background: rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.06);' }}">
                    <div class="text-2xl w-10 h-10 flex items-center justify-center rounded-lg"
                         style="{{ $badge['unlocked'] ? 'background: ' . $badge['color'] . '20;' : 'background: rgba(0,0,0,0.05);' }}">
                        {{ $badge['icon'] }}
                    </div>
                    <div class="flex-1">
                        <p class="text-sm font-semibold text-ink-800 dark:text-ink-100">{{ $badge['name'] }}</p>
                        <p class="text-xs text-ink-400">{{ $badge['desc'] }}</p>
                    </div>
                    @if($badge['unlocked'])
                    <svg class="w-5 h-5 flex-shrink-0" style="color: {{ $badge['color'] }};" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="card p-6">
            ...
            <div class="space-y-3">
                @foreach([['Cara Mendeteksi Hoaks', 'Teknik', '🔎', '#3B82F6'], ['Media Literacy 101', 'Edukasi', '📖', '#10B981'], ['Clickbait & Misinformasi', 'Analisis', '⚠️', '#EF4444']] as [$title, $cat, $icon, $color])
                <a href="{{ route('education.index') }}"
                   ...
                   onmouseout="this.style.borderColor='rgba(0,0,0,0.06)'">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-ink-800 dark:text-ink-100 truncate group-hover:text-ink-900 dark:group-hover:text-white transition-colors">{{ $title }}</p>
                        <p class="text-xs text-ink-400">{{ $cat }}</p>
                    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <a href="{{ route($action['route']) }}"
       ...
       style="background: #{{ $action['bg'] }}; border: 1px solid {{ $action['color'] }}25;">
        <div class="absolute top-0 left-0 right-0 h-px" style="background: linear-gradient(90deg, transparent, {{ $action['color'] }}60, transparent);"></div>
        ...
        <h3 class="font-display font-bold text-white text-lg mb-1">{{ $action['title'] }}</h3>
        <p class="text-sm text-ink-400">{{ $action['sub'] }}</p>
